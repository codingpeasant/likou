...
columnToIndex = {"state": 3, "timestamp": 4}


def getProfitPivotTable(column):
    resProfitBy = defaultdict(int)
    for rowId in range(1, len(input)):
        resProfitBy[input[rowId][columnToIndex[column]]] += (
            input[rowId][1] - input[rowId][2]
        )
...
def getProfitPivotTableWithTimeRange(bucketSize: int):
    timestamps = [
        int(input[rowId][columnToIndex["timestamp"]]) for rowId in range(1, len(input))
    ]
    minTimestamp, maxTimestamp = min(timestamps), max(timestamps)
    print(minTimestamp, maxTimestamp)
    buckets = [minTimestamp] + [
        int(minTimestamp + (maxTimestamp - minTimestamp) / bucketSize * (i + 1))
    ...
    for i in range(1, len(buckets)):
        ...
        while (
            rowId < len(sortedByTimestamp)
            and curBucket[0]
            <= int(sortedByTimestamp[rowId][columnToIndex["timestamp"]])
            <= curBucket[1]
        ):
