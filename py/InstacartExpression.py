    ...
    while q:
        ...
        for nei in graph[left]:
            if nei in numberRelation:
                if nei not in visited:
                    for i in range(2):  # either 0 or 1 or both should be left
                        if numberRelation[nei][i] == left:
                            numberRelation[nei][i] = right
                    if (
                        numberRelation[nei][0]
                        .lstrip("-")
                        .isdigit()  # allow negative number
                        and numberRelation[nei][1].lstrip("-").isdigit()
                    ):
                        neiRes = (
                            int(numberRelation[nei][0])
                            + int(numberRelation[nei][1]) * numberRelation[nei][2]
                        )
