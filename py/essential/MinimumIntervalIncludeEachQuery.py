...
class Solution:
    def minInterval(self, intervals: List[List[int]], queries: List[int]) -> List[int]:
        ...
        for qnum, pos in queries_asc:
            while (
                i < num_intervals
            ):  # add all the intervals that start before or at qnum
                left, right = intervals[i]
                if left > qnum:
                    break
                heapq.heappush(
                    size_heap, (right - left + 1, left)
                )  # get all the intervals that start before or at qnum
                i += 1
            ...
            while size_heap:
                ...
                if (
                    right >= qnum
                ):  # the first interval in the heap is the smallest one that contains qnum
                    break
                heapq.heappop(
                    size_heap
                )  # we don't need this interval anymore or the next iteration because the following qnum are larger than the right of this interval
