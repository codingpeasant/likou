# https://leetcode.com/problems/valid-parenthesis-string/description/?envType=problem-list-v2&envId=rr2ss0g5
# Neet
...
class Solution:
    def checkValidString(self, s: str) -> bool:
        ...
        for i in range(len(s)):
            if (
                s[i] == "("
            ):  # When encounter "(" or "*", we store it separately as "money" for future use.
                ...
                if (
                    left_par_stack
                ):  # we give priority to pay with "(", so the right-most "(" will be consumed. Greedy
                    left_par_stack.pop()
        ...
        while left_par_stack:  # In situ that some "(" haven't been consumed.
            if not star_stack:
                ...
                star_stack[-1] > left_par_stack[-1]
            ):  # Only when the idx of "*" is greater than idx of "(" that can we apply "*" as ")"
                star_stack.pop()
                left_par_stack.pop()
