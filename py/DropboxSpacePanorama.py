...
class SpacePanorama:
    ...
    def __init__(self):
        self.ordered_files = collections.OrderedDict(
            (row, col) for row in range(1000) for col in range(1000)
        )
    ...
    def write(self, image_data, row, col):
        try:
            ...
            self.ordered_files[(row, col)] = (
                None  # The value that we assign doesn't matter
            )
    ...
    def read(self, row, col):
        if not os.path.exists(f"{row}_{col}.png"):
            raise Exception(f"No image at {row}, {col}")
        with open(f"{row}_{col}.png", "rb") as file_object:
            return file_object.read()  # You can read the whole thing because it's 1 MB

    def get_last_updated_image(self):
        return (
            self.ordered_files.tail()
        )  # This isn't an actual method. Your interviewer might
