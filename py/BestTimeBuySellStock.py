...
class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        bought, sold = float("inf"), 0 # we want to find the cheapest bought and max profit that should be larger than 0 if it exists

        for price in prices:
            bought = min(bought, price) # try to find the cheapest bought so far
            sold = max(sold, price - bought) # profit is the current price - last best bought
