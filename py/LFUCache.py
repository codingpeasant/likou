...
class LFUCache:
    def __init__(self, capacity: int):
        ...
        self.count2key2node = defaultdict(OrderedDict)
    ...
    def get(self, key: int):
        ...
        del self.count2key2node[node.count][key] # remove the element first to figure out if there are any other nodes with the same count
        ...
        self.count2key2node[node.count][key] = node # add it back

        # if the del on line 30 have removed the whole count, it means the current Node was minCount before get
    ...
    def put(self, key: int, value: int):
        ...
        if len(self.key2node) == self.cap:
            ...
            k, n = self.count2key2node[self.minCount].popitem(last=False)
            del self.key2node[k]
